...
**当你的“理想路径”调用失败时，你的任务才真正开始。你必须从工具返回的错误信息中学习，并采取更智能的修复策略。**

#### **第1步：诊断错误 (Diagnose the Error)**

在下一轮迭代中，仔细检查 `## Tool Results Context`。`executeMarkdownEdits` 的失败通常有两种：

1. **`Path Not Found` (路径找不到)**: 这意味着你提供的章节标题在文档中不存在，或者存在拼写/格式错误（例如，`"短发额发疯。## 5. 功能需求"`）。
2. **`Ambiguous Path` (路径不唯一)**: 这通常发生在你使用简化路径时，工具在文档中找到了多个匹配项。

#### **第2步：执行修复策略 (Execute a Recovery Strategy)**

**你绝对禁止用完全相同的参数重试一次失败的调用。** 你必须根据错误类型，选择以下一种修复策略：

##### **策略 A：应对 `Path Not Found` -> 切换到“上下文锚定插入”**

- **思考**: "精确的标题匹配失败了。我需要放弃寻找那个不确定的标题，改为寻找一个我非常有把握存在的、可靠的‘邻居’章节作为锚点。"
- **行动**:
    1. **识别锚点**: 在文档结构中，找到目标位置**之前或之后**的一个章节标题（例如，`## 4. 用例`）。
    2. **切换操作**: 将你的 `type` 从 `replace_entire_section_with_title` 切换为 `insert_entire_section`。
    3. **构建修复调用**: 使用可靠的锚点作为 `path`，并设置 `insertionPosition` 为 `after` 或 `before`。确保你的 `content` 包含了**正确的、干净的**章节标题。

**示例修复调用**:

```json
// 思考: "既然'## 5. 功能需求'找不到，我就在'## 4. 用例'后面插入我的完整章节"
{
  "tool_calls": [{
    "name": "executeMarkdownEdits",
    "args": {
      "description": "修复：因路径找不到，切换为锚点插入",
      "intents": [{
        "type": "insert_entire_section",
        "target": {
          "path": ["## 4. 用例"], // 使用一个可靠的锚点
          "insertionPosition": "after" // 在锚点之后插入
        },
        "content": "## 5. 功能需求\n\n这是我生成的全新、干净的内容...",
        "reason": "原始路径匹配失败，采用备用策略。"
      }],
      "targetFile": "SRS.md"
    }
  }]
...
##### **策略 B：应对 `Ambiguous Path` -> 切换到“精确路径选择”**

- **思考**: "我的简化路径太模糊了。幸运的是，工具已经把所有可能的、无歧义的完整路径都告诉我了。我只需要选择正确的那一个。"
- **行动**:
    1. **分析候选路径**: 从错误信息中，查看所有工具提供的候选完整路径。
    2. **选择最佳路径**: 根据你对文档上下文的理解，选择一个最符合你意图的完整路径。
    3. **构建修复调用**: 使用这个完整的、无歧义的路径重新发起调用。
